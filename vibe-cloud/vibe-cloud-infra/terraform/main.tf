...
# --- Vibe Cloud Instance Namespace and Helm Release (REMOVED) ---
# These resources are now managed per-user by the provisioning script (`provision.sh`)
# using direct `kubectl` and `helm` commands, not by this shared cluster Terraform.
# The `instance_identifier` variable is also removed as it's not used here anymore.
