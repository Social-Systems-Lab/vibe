...
import { createHmac } from "crypto"; // Added for instanceIdFromDid
...
export function instanceIdFromDid(did: string, secret: string): string {
    ...
    // Validate DID format (specific to did:vibe with base58btc 'z' prefix)
    // Loosened the regex slightly to not mandate 'z' at the start of the multibase part,
    // as the core part of the DID is what's used for the suffix.
    // The multibase part itself starts with 'z', but the full DID is "did:vibe:z..."
    if (!/^did:vibe:z[1-9A-HJ-NP-Za-km-z]+$/i.test(did)) {
        // logger.warn(`Invalid DID format for instanceId generation: ${did}`); // Optional logging
        throw new Error("Input does not look like a valid did:vibe DID for instanceId generation.");
    }

    /* ---------- 1. salted, deterministic digest ---------- */
    // HMAC-SHA-256(did, secret)  → 32-byte buffer
    const digest = createHmac("sha256", secret).update(did).digest();

    /* ---------- 2. base-58 encode (multibase) and force lower-case for prefix ---------- */
    // Use first 12 bytes of digest for primary part of prefix
    const multibaseEncodedBytesPrimary = multibase.encode("base58btc", digest.subarray(0, 12));
    // Decode to string, remove 'z' prefix from multibase, and convert to lowercase
    const rawBase58Primary = new TextDecoder().decode(multibaseEncodedBytesPrimary).substring(1).toLowerCase();

    // Use next bytes of digest for padding if primary part is too short
    ...
    // Pad primary part to PREFIX_LEN using secondary part, then slice to ensure exact length
    ...
    // Take last SUFFIX_LEN characters of the full DID string and convert to lowercase
