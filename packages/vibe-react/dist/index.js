...
"use client";
var VibeContext = import_react.createContext(undefined);
var getSdk = () => {
  return window.vibe;
};
function VibeProvider({ children, manifest }) {
  const [vibeState, setVibeState] = import_react.useState(undefined);
  const [sdkUnsubscribe, setSdkUnsubscribe] = import_react.useState(null);
  ...
  const init = import_react.useCallback(() => {
    console.warn("[VibeProvider] Manual init called. SDK should auto-initialize via AgentProvider.");
  }, []);
  const readOnce = import_react.useCallback(async (collection, filter) => {
    const sdk = getSdk();
    if (!sdk)
  ...
  const read = import_react.useCallback(async (collection, filter, callback) => {
    const sdk = getSdk();
    if (!sdk)
      throw new Error("Vibe SDK (window.vibe) not available.");
  ...
  const write = import_react.useCallback(async (collection, data) => {
    const sdk = getSdk();
    if (!sdk)
      throw new Error("Vibe SDK (window.vibe) not available.");
  ...
  return /* @__PURE__ */ jsx_dev_runtime.jsxDEV(VibeContext.Provider, {
    value: contextValue,
    children
  }, undefined, false, undefined, this);
}
function useVibe() {
  const context = import_react.useContext(VibeContext);
