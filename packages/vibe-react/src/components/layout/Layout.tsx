...
export function Layout({
    ...
    for (const el of arr) {
        ...
        const isNav =
            lname.includes("navpanel") ||
            el.type === NavPanelCmp ||
            (typeof el.type === "function" && (el.type as any).__isNavPanel === true);
        if (!navChild && isNav) {
            navChild = el;
            continue;
        }
                ...
                <div className="grid w-full" style={{ gridTemplateColumns: `var(--left-col-width) minmax(0, 1fr)` }}>
                    ...
                    <div
                        className={`hidden md:flex flex-col ${
                            variantConfig.chrome?.dashboard?.leftBorder !== false ? "border-r border-gray-200" : ""
                        }`}
                        style={{ gridColumn: "1 / 2" }}
                    >
                        {headerForLeft}
                        {navNode ? (
                            <div className="mt-2 flex-1">{navNode}</div>
