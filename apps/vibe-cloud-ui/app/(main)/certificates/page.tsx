      ...
      try {
        ...
        if (ids.length > 0) {
          const revRes = await fetch(`${apiBase}/data/types/revocations/query`, {
            method: "POST",
            headers: authHeaders,
            body: JSON.stringify({ selector: { certId: { $in: ids } } }),
          });
          if (revRes.ok) {
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`inline-flex items-center rounded-md px-3 py-1.5 text-sm border ${
                activeTab === t ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border hover:bg-accent/20"
              }`}
            >
              {t}
            </button>
