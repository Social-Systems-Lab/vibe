...
export class QuotaService {
    ...
    constructor() {
        ...
        const limitMb = Number(process.env.QUOTA_DEFAULT_LIMIT_MB) || Number(process.env.LEVEL_BASE_CAP_MB) || 512;
    ...
    async reconcile(userDid: string): Promise<{ released: number; finalReserved: number }> {
        const user = await this.findUserByDid(userDid);
        if (!user) {
            return { released: 0, finalReserved: 0 };
        }
        let released = 0;
        await this.casUpdateById(user._id, (doc) => {
            const q: Quota = this.initQuotaIfMissing(doc) && (doc.quota as Quota);
            if (!q.reservations) return;
            const now = new Date();
            for (const uploadId in q.reservations) {
                const r = q.reservations[uploadId];
                if (r && new Date(r.expiresAt) < now) {
                    const size = r.size || 0;
                    q.reserved_bytes = Math.max(0, (q.reserved_bytes || 0) - size);
                    released += size;
                    delete q.reservations[uploadId];
                }
            }
            q.updatedAt = nowIso();
        });
        const finalQuota = (user.quota || {}) as Quota;
        return { released, finalReserved: finalQuota.reserved_bytes || 0 };
    }
}
